...
const unsigned long PRINT_INTERVAL = 20; // 50Hz
...
void calibrateMPU() {
  ...
  for (int i = 0; i < samples; i++) {
   ...
   //Full +- 180
sumPitch += atan2(-a.acceleration.x, sqrt(a.acceleration.y*a.acceleration.y + a.acceleration.z*a.acceleration.z)) * 180.0 / PI;
sumRoll  += atan2( a.acceleration.y, sqrt(a.acceleration.x*a.acceleration.x + a.acceleration.z*a.acceleration.z)) * 180.0 / PI;
...
// Accel angles +- 180
double accelPitch = atan2(-a.acceleration.x, sqrt(a.acceleration.y*a.acceleration.y + a.acceleration.z*a.acceleration.z)) * 180.0 / PI - pitchOffset;
double accelRoll  = atan2( a.acceleration.y, sqrt(a.acceleration.x*a.acceleration.x + a.acceleration.z*a.acceleration.z)) * 180.0 / PI - rollOffset;

// Gyro rates deg/s
double gyroRateY = g.gyro.y * 180.0 / PI;
double gyroRateX = g.gyro.x * 180.0 / PI;

// ADD HERE — right after gyro rate calculation:
if (abs(gyroRateY) < 1.0) gyroRateY = 0;
if (abs(gyroRateX) < 1.0) gyroRateX = 0;
...
if (now - lastPrintTime >= PRINT_INTERVAL) {
  ...
  // ✅ Optimized: multiply floats to int16 to save bytes
  // Accel: m/s² × 100 → int16  (range ±4g = ±39.2 m/s², fits int16)
  // Gyro:  deg/s × 10 → int16  (range ±500 deg/s, fits int16)
  // Pitch/Roll: degrees × 100 → int16 (range ±180°, fits int16)

  int16_t iPitch = (int16_t)(pitch * 100);
  int16_t iRoll  = (int16_t)(roll  * 100);
  int16_t iAx    = (int16_t)(a.acceleration.x * 100);
  int16_t iAy    = (int16_t)(a.acceleration.y * 100);
  int16_t iAz    = (int16_t)(a.acceleration.z * 100);
  int16_t iGx    = (int16_t)(g.gyro.x * 180.0 / PI * 10);
  int16_t iGy    = (int16_t)(g.gyro.y * 180.0 / PI * 10);
  int16_t iGz    = (int16_t)(g.gyro.z * 180.0 / PI * 10);

  // Pack into byte buffer
  // Format: ts(4) + pitch(2) + roll(2) + ax(2) + ay(2) + az(2) + gx(2) + gy(2) + gz(2) = 20 bytes
  uint8_t buf[20];
  memcpy(buf,      &ts,     4);
  ...
  Serial.print(g.gyro.x * 180.0/PI, 2); Serial.print(",");
  Serial.print(g.gyro.y * 180.0/PI, 2); Serial.print(",");
  Serial.println(g.gyro.z * 180.0/PI, 2);
